...
const getPageMetaData = async (post) => {
  ...
  console.log(post.properties["Featured image"].files[0].file.url, path.join(process.cwd(), `/public/images/posts/${post.id}.${ext}`))
  if (!fs.existsSync(path.join(process.cwd(), `/public/images/posts/${post.id}.${ext}`))) {
    await downloadImage(post.properties["Featured image"].files[0].file.url, path.join(process.cwd(), `/public/images/posts/${post.id}.${ext}`))
  }
  return new Promise(resolve => resolve({
    ...
    featuredImage: `/images/posts/${post.id}.${ext}`,
