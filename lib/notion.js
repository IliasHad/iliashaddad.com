...
const getPageMetaData = async (post) => {
  ...
  if (!fs.existsSync('.next/static/chunks/images')) {
    fs.mkdirSync('.next/static/chunks/images')
  }
  if (!fs.existsSync(path.resolve('.next/static/chunks/images', `${post.id}.${ext}`))) {
    await downloadImage(post.properties["Featured image"].files[0].file.url, path.resolve('.next/static/chunks/images', `${post.id}.${ext}`))
  }
  return new Promise(resolve => resolve({
    ...
    featuredImage: `/.next/static/chunks/images/${post.id}.${ext}`,
