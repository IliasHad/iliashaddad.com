...
export const getAllSideProjects = async () => {
  ...
  return allSideProjects.map((sideProject) => {
    return getPageMetaData(sideProject);
  });
};
export const getAllClientsProjects = async () => {
  ...
  return allClientProjects.map((clientProject) => {
    return getPageMetaData(clientProject);
  });
};
...
export const getRecentFeaturedPost = async () => {
  ...
  return getPageMetaData(posts.results[0]);
};
const getPageMetaData = (post) => {
  ...
  return {
    ...
    featuredImage: post.properties["Featured image"].files[0].file.url,
